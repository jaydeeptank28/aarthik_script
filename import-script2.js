...
function buildAdditionalInfo(row, skip) {
    ...
    for (const key in row) {
        ...
        arr.push(`${toCamelCase(key)} : ${row[key]}`);
    }
    return arr.join(" | ");
}
...
async function importFile(filePath) {
    ...
    for (const sheetName of workbook.SheetNames) {
        ...
        if (!rows.length) {
            ...
            await updateLog(logId, { total_rows: 0, inserted_rows: 0, failed_rows: 0 },
                "failed", sheetEnd - sheetStart);
            results.push({ sheet: sheetName, success: false, error: "Empty sheet", log_id: logId });
            continue;
        }
        ...
        for (const r of rows) {
            index++;
            const row = {};
            for (const k in r) row[k.toLowerCase()] = r[k];
